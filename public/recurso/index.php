...
<html lang="en">
  ...
  <body style="background-color: #eff3f9;">
    ...
    <a class="navbar-brand col-md-3 col-lg-2 me-0 px-3 fs-6" href="#">TCC Andiara - Gestão de Recursos </a>
        ...
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block sidebar collapse" style="background-color: #eff3f9;">
          <div class="position-sticky pt-3 sidebar-sticky">
            <ul class="nav flex-column">
              <li class="nav-item">
                <a class="nav-link active" aria-current="page" href="/recurso/">
            ...
            <div class="col-12">
              ...
              <div class="">
                <?php
                  ...
                  $parameters = $Recurso->to_datatable($list);
              ...
              <br>
              <hr>
              <div class="modal fade modal-lg" id="modal_form_recurso" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title" id="exampleModalLabel">Cadastro de Recurso</h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      <form id="recurso" name="recurso" class="form form-group form-sm" >
                      </form>
                    </div>
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                      <button type="button" class="btn btn-primary btn_submit_form" id="btn_submit_form" data-form_id="recurso">Confirmar</button>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <script>
      $(document).ready(function(){
        ...
        // buscar o formulário
        $.get("/api/?api=recurso&action=get_form", function(data){
          console.log(data);
          if ( data.fields ){
            $.each(data.fields, function(k,v){
              $("#recurso").append(gerar_campo(v))
            })
          }
        });
      
        $("#btn_create").on("click", function(){
          let form = $("form#recurso");
          $(form).trigger("reset");
        ...
        $(".btn_edit").on("click", function(){
          ...
          console.log(id);  
          $.get(`/api/?api=recurso&action=list&id=${id}`, function(data){
            console.warn(data);
            if ( data[0].id ){              
              $(form).trigger("reset");
