...
<html lang="en">
  ...
  <body style="background-color: #eff3f9;">
    ...
    <a class="navbar-brand col-md-3 col-lg-2 me-0 px-3 fs-6" href="#">TCC Andiara - Recursos x Contratos</a>
        ...
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block sidebar collapse" style="background-color: #eff3f9;">
          <div class="position-sticky pt-3 sidebar-sticky">
            <ul class="nav flex-column">
              ...
              <li class="nav-item">
                <a class="nav-link active" href="/recurso_contrato/">
                ...
                <div class="modal-dialog modal-lg" role="document">
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <form id="recursos_contrato" name="recursos_contrato" class="form form-group form-sm" >
                      </form>
                    </div>
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                      <button type="button" class="btn btn-primary btn_submit_form" id="btn_submit_form" data-form_id="recursos_contrato">Confirmar</button>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <script>
      $(document).ready(function(){
        ...
        // buscar o formulário
        $.get("/api/?api=recursos_contrato&action=get_form", function(data){
          console.log(data);
          if ( data.fields ){
            $.each(data.fields, function(k,v){
              $("#recursos_contrato").append(gerar_campo(v))
            })
          }
        });
      
        $("#btn_create").on("click", function(){
          let form = $("form#recursos_contrato");
          $(form).trigger("reset");
        ...
        $(".btn_edit").on("click", function(){
          ...
          console.log(id);  
          $.get(`/api/?api=recursos_contrato&action=list&id=${id}`, function(data){
            console.warn(data);
            if ( data[0].id ){              
              $(form).trigger("reset");
        ...
        function submit_form(form_name, action){
          ...
          formdata.append(`api`, `recursos_contrato`);
