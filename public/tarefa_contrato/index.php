...
<html lang="en">
  <head>
    ...
    <title>Tarefas x Contratos</title>
    <!-- Bootstrap core CSS -->
  ...
  <body style="background-color: #eff3f9;">
    ...
    <a class="navbar-brand col-md-3 col-lg-2 me-0 px-3 fs-6" href="#">TCC Andiara - Tarefas x Contratos </a>
        ...
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block sidebar collapse" style="background-color: #eff3f9;">
          <div class="position-sticky pt-3 sidebar-sticky">
            <ul class="nav flex-column">
              ...
              <li class="nav-item">
                <a class="nav-link active " href="/tarefa_contrato/">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
          <div class="row">
            <div class="col-12">
              ...
              <br>
              <hr>
              <div class="modal fade modal-lg" id="modal_form_tarefa" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg" role="document">
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <form id="tarefas_contrato" name="tarefas_contrato" class="form form-group form-sm" >
                      </form>
                    </div>
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                      <button type="button" class="btn btn-primary btn_submit_form" id="btn_submit_form" data-form_id="tarefas_contrato">Confirmar</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <br>
              <hr>
              <div class="modal fade modal-lg" id="modal_form_tarefa_upload" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title" id="exampleModalLabel">Uplaods</h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      <form id="tarefas_contrato_upload" name="tarefas_contrato_upload" class="form form-group form-sm" >
                      </form>
                    </div>
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                      <button type="button" class="btn btn-primary btn_submit_form" id="btn_submit_form" data-form_id="tarefas_contrato_upload">Confirmar</button>
                    </div>

                    <div class="modal-body">
                      <div class="col lista_uploads"></div>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <script>
      $(document).ready(function(){
        ...
        // buscar o formulário
        $.get("/api/?api=tarefas_contrato&action=get_form", function(data){
          console.log(data);
          if ( data.fields ){
            $.each(data.fields, function(k,v){
              $("#tarefas_contrato").append(gerar_campo(v))
            })
          }
        });

        // buscar o formulário upload
        $.get("/api/?api=tarefas_contrato&action=get_form_upload", function(data){
          console.log(data);
          if ( data.fields ){
            $.each(data.fields, function(k,v){
              $("#tarefas_contrato_upload").append(gerar_campo(v))
            })
          }
        });
      
        $("#btn_create").on("click", function(){
          let form = $("form#tarefas_contrato");
          $(form).trigger("reset");
        ...
        $(".btn_edit").on("click", function(){
          ...
          console.log(id);  
          $.get(`/api/?api=tarefas_contrato&action=list&id=${id}`, function(data){
            console.warn(data);
            if ( data[0].id ){              
              $(form).trigger("reset");
        ...
        $(".btn_upload").on("click", function(){
          ...
          let action = $(this).data("action");
          $(form).closest(".modal").modal("show");
          $(form).closest(".modal").find("#btn_submit_form").data("action", "upload");
          $(form).find(`:input[name='id_tarefa_contrato']`).val(id);
          $(form).data("action", "upload");
          console.log(form);
          console.log(id);  
          $.get(`/api/?api=tarefas_contrato&action=list_datatable_uploads&id_tarefa_contrato=${id}`, function(data){
            console.warn(data);
            if ( data.lista_uploads ){              
              $(form).closest(".modal").find(".lista_uploads").html(data.lista_uploads);
            }
          })
        });
        ...
        function submit_form(form_name, action){
          ...
          formdata.append(`api`, `tarefas_contrato`);
          ...
          $.ajax({
            ...
            switch (form_name){
              case "tarefas_contrato_upload":
                ...
                let id_tarefa_contrato = $(form_upload).find(":input[name='id_tarefa_contrato']").val();
                $.get(`/api/?api=tarefas_contrato&action=list_datatable_uploads&id_tarefa_contrato=${id_tarefa_contrato}`, function(data){
                  console.warn(data);
                  if ( data.lista_uploads ){              
                    $(form_upload).closest(".modal").find(".lista_uploads").html(data.lista_uploads);
                  }
                })
                break;
        ...
        $(document).on("click", ".delete_upload", function(){
          let object_info = $(this).data("object_info");
          let text = `O cadastro de ${object_info.nome} será excluído e não poderá ser recuperado`
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              $.ajax({
                ...
                //data.popup.reload = true;
                popup("success", data.popup);
                $.get(`/api/?api=tarefas_contrato&action=list_datatable_uploads&id_tarefa_contrato=${object_info.id_tarefa_contrato}`, function(data){
                  console.warn(data);
                  if ( data.lista_uploads ){              
                    $("form#tarefas_contrato_upload").closest(".modal").find(".lista_uploads").html(data.lista_uploads);
                  }
                })
              });
            } 
